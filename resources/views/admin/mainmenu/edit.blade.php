...
@section('content')
    ...
    <div class="col-lg-6">
        ...
        @endif
        <form action="{{ url('admin/mainmenu/edit/'.$data->id) }}" method="POST" enctype="multipart/form-data" id="form">
            @csrf
            ...
            <select name="parent" id="parent" class="form-control">
                ...
                @foreach ($parent as $par)
                    <option value="{{ $par->id }}" {{ ($par->id == $data->parent) ? 'selected' : '' }}>{{ $par->title }}</option>
                @endforeach
            </select><br>
